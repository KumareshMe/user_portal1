class UserPortal < ApplicationRecord
  ...
  def self.search(params)
    ...
    user = user.where('first_name ILIKE ?', "%#{params[:first_name]}%") if params[:first_name].present?
    user = user.where('last_name ILIKE ?', "%#{params[:last_name]}%") if params[:last_name].present?
